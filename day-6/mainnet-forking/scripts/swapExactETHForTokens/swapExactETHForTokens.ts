...
const main = async () => {
  ...
  const amountOutMin = ethers.parseUnits("1500", 6);
 ...
 const wethBalanceBefore = await ethers.provider.getBalance(
  ...
  //  await WETH.approve(UNIRouter, amountOutMin);
...
const wethBalanceAfter = await ethers.provider.getBalance(
    impersonatedSigner
  );
    console.log("dai balance after", Number(wethBalanceAfter));
  console.log("usdc balance after", Number(usdcBalanceAfter));

  console.log("=========Difference==========");
  const newUsdcValue = Number(usdcBalanceAfter - usdcBalanceBefore);
  const newWethValue = wethBalanceBefore - wethBalanceBefore ;
